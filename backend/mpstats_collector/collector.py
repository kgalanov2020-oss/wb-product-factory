...
class PlaywrightMPStatsCollector:
    ...
    async def _login(self, page: Any, storage_path: Path) -> None:
        ...
        await page.locator(self._settings.mpstats_email_selector).fill(self._settings.mpstats_email)
        await page.locator(self._settings.mpstats_password_selector).fill(password.get_secret_value())
        await page.locator(self._settings.mpstats_submit_selector).click()
        await page.wait_for_load_state("networkidle")
